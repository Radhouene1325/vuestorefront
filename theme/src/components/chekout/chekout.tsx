...
import DeliveryOptions from "@/components/chekout/methodePyment";
import PaymentMethodsType from "@/components/chekout/TypePyment";
...
export default function Chekout({ ...rest}) {
    console.log(rest)
                ...
                <div className="px-4 pb-4 mt-3 md:px-6 md:pb-6 md:mt-0">
                    <div className="flex justify-between typography-text-base pb-4">
                        <div className="flex flex-col grow pr-2">
                            ...
                            <p className="typography-text-xs text-secondary-700">{rest?.prices?.discounts?.map((e:any)=>{return e.label})}</p>
                            <p className="my-2">Delivery</p>
                            <p>Estimated Sales Tax</p>
                        </div>
                        <div className="flex flex-col text-right">
                            <p>{rest?.prices?.subtotal_including_tax.currency}:{rest?.prices?.subtotal_including_tax.value}</p>
                            <p className="typography-text-xs text-neutral-500">{rest?.prices?.grand_total.currency}:{rest?.prices?.grand_total.value}</p>
                            <p className="typography-text-xs text-secondary-700">{rest?.prices?.discounts?.map((e:any)=>{return e.amount.value})}</p>
                            <p className="my-2">{formatPrice(orderDetails.delivery)}</p>
                            <p>{rest?.prices?.grand_total.currency}:{rest?.prices?.grand_total.value}</p>
                        </div>
                    </div>
                    {promoCode ? (
                    ...
                    {/*{*/}
                    {/*    rest.available_payment_methods?.map((payment_method  ,index) => {return  <DeliveryOptions payment_method={payment_method} key={index} {...rest} />})*/}
                    {/*}*/}
                    <DeliveryOptions available_payment_methods={rest.available_payment_methods} setCheckedState={setCheckedState} checkedState={checkedState}  />
                    {checkedState==='stripe_payments'&&<Elements
                        options={{
                            mode: 'payment',
                            amount: convertToSubCurrency(rest.amount),
                            currency: 'usd',
                            paymentMethodCreation: 'manual'
                        }}
                        stripe={rest.stripePromise}
                    >
                        <ChekoutElement amount={rest.amount}  adress={rest.data}/>
