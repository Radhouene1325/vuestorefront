...
const handleError = (err) => {
    if (err.cause?.code === "ECONNREFUSED")
        return new SDKError("Please check if the middleware is running and is accessible by frontend app", err);
...
const defaultLogger = {
    ...
    onResponse: ({ config, url, responseTime }) => {
        const { pathname } = new URL(url);
        internalLogger.debug(`${config.method} ${pathname} in ${responseTime.toFixed()}ms`);
    },
};
...
const getRequestSender = (options) => {
    const { apiUrl, ssrApiUrl, defaultRequestConfig = {}, methodsRequestConfig = {}, cdnCacheBustingId = undefined, } = options;
    ...
    const getConfig = (config, methodConfig) => {
        ...
        const mergedHeaders = Object.fromEntries(Object.entries({
            ...defaultHeaders,
            ...methodHeaders,
            ...headers,
        }).reduce((headersSoFar, [key, value]) => 
    ...
    const defaultHTTPClient = async (url, params, config) => {
        ...
        if (!response.ok) {
            const message = `${config?.method} ${url} ${responseJson?.message ?? response.statusText}`;
...
export { SdkHttpError, buildModule, eventManager, getRequestSender, handleError, initSDK, initVSFSDK, isCausedBySdkHttpError, isSdkRequestError, isSdkUnauthorizedError, isSpecificSdkHttpError, middlewareModule, prepareConfig };
